...
class GoalPlane(gym.Env):
    ...
    def reset(self):
        ...
        if self.goals is None:
            ...
            while condition:  # note: goal should not be in the block
                #self.goal = np.array([self.maze_size / 2 + (0.5-np.random.random())*5, self.maze_size / 6 + (0.5-np.random.random())*5]) # self.goal_space.sample()
    ...
    def render(self, mode='rgb_array', subgoal=None):
        ...
        if subgoal is not None:
            subgoal_loc = copy.copy(subgoal)
            subgoal_loc[0] = subgoal_loc[0] / self.maze_size * image.shape[1]
            subgoal_loc[1] = subgoal_loc[1] / self.maze_size * image.shape[0]
            cv2.circle(image, (int(subgoal_loc[0]), int(subgoal_loc[1])), 8, (255, 255, 0), -1)
